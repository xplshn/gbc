...
type Symbol struct {
	Name        string
	Type        SymbolType
	QbeName     string // The name used in the QBE IR (e.g., $.foo, %.foo.1)
	IsVector    bool
	IsByteArray bool
	StackOffset int64  // For auto variables
	Next        *Symbol
}

// Scope represents a lexical scope, containing a linked list of symbols
type Scope struct {
	Symbols *Symbol
	Parent  *Scope
}

// StringEntry tracks a unique string literal and its generated label
type StringEntry struct {
	Value string
	Label string
}

// AutoVarInfo is used during the function analysis pass to calculate stack layout
// It holds pre-calculated information about a local variable.
type AutoVarInfo struct {
	Node *AstNode
	Size int64 // Size in bytes
}

// CodegenContext holds all state required for the code generation pass
type CodegenContext struct {
	out              strings.Builder // Buffer for QBE IR
	asmOut           strings.Builder // Buffer for inline assembly
	strings          []StringEntry
	tempCount        int
	labelCount       int
	currentScope     *Scope
	currentFuncFrame string // QBE name for the stack frame base pointer
	breakLabel       string
	wordType         string // "l" for 64-bit, "w" for 32-bit
	wordSize         int    // 8 for 64-bit, 4 for 32-bit
	warnings         bool   // Flag to enable/disable warnings
	phiFromLabel     string // Tracks the correct predecessor label for a phi node
}

// NewCodegenContext initializes a code generation context.
func NewCodegenContext() *CodegenContext {
	var ws int
	var wt string
	// Determine word size and type based on host architecture (this approach sucks)
...
func (ctx *CodegenContext) addSymbol(name string, symType SymbolType, isVector bool, tok Token) *Symbol {
	if ctx.findSymbolInCurrentScope(name) != nil {
		if symType == SYM_VAR {
			// B allows redeclaration, handled in the parser/analysis passes
		}
	}
	...
	case SYM_VAR:
		if ctx.currentScope.Parent == nil { // Global scope
			qbeName = "$" + name
		} else { // Local scope - generate a unique temporary name for its address
...
// collectGlobals recursively walks the top-level AST to find all global declarations
// (variables, functions, extrn, __asm__) and adds them to the global symbol table
func (ctx *CodegenContext) collectGlobals(node *AstNode) {
	...
	switch node.Type {
	case AST_BLOCK:
		// Recurse into blocks to find all top-level declarations,
		// including those in synthetic blocks from `extrn` lists.
		for _, stmt := range node.Data.(AstBlock).Stmts {
			ctx.collectGlobals(stmt)
		}
	case AST_VAR_DECL:
		// Only add global variables. Local (auto) variables are handled during function codegen.
		if ctx.currentScope.Parent == nil {
			d := node.Data.(AstVarDecl)
			// Avoid redefining a symbol if it was already declared (e.g., as extrn)
	...
	case AST_FUNC_DECL:
		...
		// Avoid redefining a symbol
		if ctx.findSymbolInCurrentScope(d.Name) == nil {
			ctx.addSymbol(d.Name, SYM_FUNC, false, node.Tok)
		}
	case AST_EXTRN_DECL:
		d := node.Data.(AstExtrnDecl)
		// In B, `extrn` can be for variables or functions, if the symbol is called, we know that it is a function, else, it works a fordward declaration
		if ctx.findSymbolInCurrentScope(d.Name) == nil {
			ctx.addSymbol(d.Name, SYM_FUNC, false, node.Tok)
		}
	}
...
func (ctx *CodegenContext) findByteArrays(root *AstNode) {
	...
	walk = func(n *AstNode) {
		...
		if n.Type == AST_ASSIGN {
			d := n.Data.(AstAssign)
			if d.Lhs.Type == AST_SUBSCRIPT && d.Rhs.Type == AST_INDIRECTION {
				lhsSub := d.Lhs.Data.(AstSubscript)
				rhsIndir := d.Rhs.Data.(AstIndirection)
				if lhsSub.Array.Type == AST_IDENT && isStringDerivedExpr(rhsIndir.Expr) {
					arrayName := lhsSub.Array.Data.(AstIdent).Name
					sym := ctx.findSymbol(arrayName)
					if sym != nil {
...
func (ctx *CodegenContext) codegenLogicalCond(node *AstNode, trueL, falseL string) {
	...
	// Explicitly check if the condition value is non-zero. This creates a
	// canonical boolean value (0 or 1) as a long, which is safer than
	// relying on the result of a complex expression being a clean 0 or 1.
	...
	// On 64-bit targets, the result of the comparison is a 'long'. The 'jnz'
	// instruction, however, strictly requires a 'word' argument. We must
	// explicitly truncate the long to a word before the jump.
	if ctx.wordType == "l" {
		condValWord := ctx.newTemp()
		ctx.out.WriteString(fmt.Sprintf("\t%s =w copy %s\n", condValWord, isNonZero))
		condValForJnz = condValWord
	} else {
		// On 32-bit targets, the value is already a 'word'.
...
func (ctx *CodegenContext) codegenExpr(node *AstNode) (result string, terminates bool) {
	...
	case AST_IDENT:
		...
		if sym == nil {
			// Implicitly declare external functions if not found.
	...
	case AST_INDIRECTION:
		addr, _ := ctx.codegenExpr(node.Data.(AstIndirection).Expr)
		res := ctx.newTemp()
		loadInstruction := "load" + ctx.wordType
		if isStringDerivedExpr(node.Data.(AstIndirection).Expr) {
			loadInstruction = "loadsb"
		}
	...
	case AST_FUNC_CALL:
		...
		isStmt := node.Parent != nil && (node.Parent.Type == AST_BLOCK || node.Parent.Type == AST_IF || node.Parent.Type == AST_WHILE || node.Parent.Type == AST_FUNC_DECL)
		...
		if d.FuncExpr.Type == AST_IDENT {
			funcName := d.FuncExpr.Data.(AstIdent).Name
			if funcName == "abort" || funcName == "exit" {
				ctx.out.WriteString("\thlt\n")
				return res, true
			}
		}
		return res, false
	...
	case AST_TERNARY:
		...
		res := ctx.newTemp() // The temporary for the phi node result

		ctx.codegenLogicalCond(d.Cond, thenLabel, elseLabel)

		// Then branch
		ctx.out.WriteString(thenLabel + "\n")
		// Set the default predecessor label for the 'then' branch.
		// This will be updated by a nested ternary if one exists
		ctx.phiFromLabel = thenLabel
		thenVal, thenTerminates := ctx.codegenExpr(d.ThenExpr)
		// Capture the potentially updated predecessor label
		...
		// Set the default predecessor label for the 'else' branch
		...
		if !thenTerminates || !elseTerminates {
			...
			// Update the context's predecessor label to this ternary's end label,
			// for any outer ternaries
			ctx.phiFromLabel = endLabel

			return res, thenTerminates && elseTerminates
		}

		// Both branches terminate (e.g., by calling exit()), so the code is unreachable
		return "0", true
	}
...
func (ctx *CodegenContext) codegenFuncDecl(node *AstNode) {
	...
	for _, local := range allLocals {
		var sym *Symbol
		if local.Node.Type == AST_IDENT { // Parameter
			p := local.Node
